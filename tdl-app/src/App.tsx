...
const ingredientImages = require.context('./components/Images', true)
...
function App() {
  ...
  const combineIngredients = (selectedIngredients: ingredient[]) => {
    if (selectedIngredients.length===0) {
      swal(`At least one ingredient must be selected.`)
      return;
    }
    ...
    if(result !== undefined){
      ...
      if (oldRecipe === undefined) {
        swal({text:"No combination exists", icon:"warning"})
      } else {
        if (oldRecipe.isAvailable === true) {
          swal({text:`You already discovered ${resultName}.`, icon:"warning"})
          return;
        }
        ...
        Swal.fire({text:`Congrats! You discovered ${resultName} 👨‍🍳`, imageUrl:ingredientImages(`./${ resultName }.png`), imageHeight:126, imageWidth:105})
      }
    }
    else 
      swal({text:"No combination exists", icon:"warning"})
